...
import { L2GatewayToken__factory } from '@arbitrum/sdk/dist/lib/abi/factories/L2GatewayToken__factory'
...
} from '@arbitrum/sdk/dist/lib/dataEntities/errors'
import { AlephZeroChainConfig } from '../util/alephChainsList'
import {
  GasOverrides,
  ParentToChildMessageGasEstimator
} from '@arbitrum/sdk/dist/lib/message/ParentToChildMessageGasEstimator'
import { isArbitrumChain } from '@arbitrum/sdk/dist/lib/utils/lib'
// import {
//   GasOverrides,
//   ParentToChildMessageGasEstimatorCustom
// } from './ParentToChildGasEstimatorCustom'
 ...
 */
export class Erc20BridgerGifter extends AssetBridger<
...
> {
  ...
  public constructor(childNetwork: ArbitrumNetwork) {
    ...
    this.gifter = (childNetwork as AlephZeroChainConfig).tokenBridge.gifter
  }

  /**
   * Instantiates a new Erc20BridgerGifter from a child provider
   * @param childProvider
   * @returns
   */
  public static async fromProvider(childProvider: Provider) {
    return new Erc20BridgerGifter(await getArbitrumNetwork(childProvider))
  }
  ...
  public async getApproveTokenRequest(
    ...
    const data = iErc20Interface.encodeFunctionData('approve', [
      // you approve tokens to the gifter, gifter approves to the gateway that router will use
      this.gifter,
      params.amount || Erc20BridgerGifter.MAX_APPROVAL
    ])
    ...
    if (
      ...
      if (!tokenGasOverrides.gasLimit.min) {
        tokenGasOverrides.gasLimit.min =
          Erc20BridgerGifter.MIN_CUSTOM_DEPOSIT_GAS_LIMIT
      }
    }
  ...
  public async deposit(
    ...
    const erc20ParentAddress = isParentToChildTransactionRequest(params)
      ? getErc20ParentAddressFromParentToChildTxRequestCustom(params)
 ...
 */
export class AdminErc20Bridger extends Erc20BridgerGifter {
...
export const getErc20ParentAddressFromParentToChildTxRequestCustom = (
